...
class PythonVirtualenvTestConan(ConanFile):
    ...
    # limitations in 1.x
    python_requires = f"cmake-python-deps/{get_version()}@mtolympus/stable"
    ...
    def test(self):
        if not cross_building(self):
            bindir = os.path.join("test_package", self.cpp.build.bindirs[0]) if self.settings.os == "Windows" else os.path.join(self.cpp.build.bindirs[0], 'test_package')
            cmd = f"{os.path.join(bindir, 'dumpfile')} 20 {os.path.join(self.build_folder, 'test_package', 'docs', 'sphinx' ,'index.html')}"
